...
interp.bilin_interp2(image, x, y, z)

z = blt.bilin_interp(image, x, y)

x = np.full([16129], 10.3, dtype=np.double)
y = np.full([16129], 10.1, dtype=np.double)
z = np.full([16129], 0, dtype=np.double)
...
mywrap2 = wrapper(interp.bilin_interp1, image, x, y)
timeit(mywrap2, number=1000)

mywrap3 = wrapper(interp.bilin_interp2, image, x, y, z)
