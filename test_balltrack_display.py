#%gui qt
from importlib import reload
import numpy as np
from scipy import interpolate
import matplotlib.pyplot as plt
import balltracking.balltrack as blt
import fitstools
import fitsio
import filters

from mayavi import mlab


def wrapper(func, *args, **kwargs):
    def wrapped():
        return func(*args, **kwargs)
    return wrapped



# Path to fits file (fits cube)
#file    = '/Users/rattie/Data/SDO/HMI/EARs/AR11130_2010_11_27/mtrack_20101126_170034_TAI_20101127_170034_TAI_LambertCylindrical_continuum.fits'
file = '/Users/rattie/Data/SDO/HMI/EARs/AR12673_2017_09_01/series_continuum/mtrack_20170901_000000_TAI20170905_235959_LambertCylindrical_continuum_00000.fits'
# Get the header
h       = fitstools.fitsheader(file)
# Get the 1st image
image   = fitsio.read(file)
image = image.astype(np.double)
# Filter image
ffilter_hpf = filters.han2d_bandpass(image.shape[0], 0, 5)
fdata_hpf = filters.ffilter_image(image, ffilter_hpf)
sigma0 = image[1:200, 1:200].std()
sigma_hpf = fdata_hpf[1:200, 1:200].std()

surface0 = blt.rescale_frame(image, sigma0)
surface_hpf = blt.rescale_frame(fdata_hpf, 2*sigma_hpf)


#surface = np.zeros([30, 60])
nt = 10
rs = 2
dp = 0.2
bt = blt.BT(surface0.shape, nt, rs, dp)
# Initializa ball positions with height
bt.initialize_ballpos(surface_hpf)


bsize = np.ones([bt.xstart.ravel().shape[0]], dtype=float)*4

skip = 50

# Background colors
blue=(0.16, 0.28, 0.46)
gray=(0.5, 0.5, 0.5)
# Display the data surface as a surface and with the 3d wireframe mesh.
f0 = mlab.figure(size=(900, 900), bgcolor=gray)
s0 = mlab.surf(bt.xcoords, bt.ycoords, surface_hpf, warp_scale=1.0, vmin=-2, vmax=2, colormap='afmhot', line_width=1, opacity=1)
s0.actor.mapper.interpolate_scalars_before_mapping = True
s0.actor.property.edge_visibility = True
# Add the balls as spherical glyphs with proper radius
pts0 = mlab.quiver3d(bt.xt[::skip, 0], bt.yt[::skip, 1], bt.zt[::skip, 2], bsize[::skip], bsize[::skip], bsize[::skip], scale_factor=1.0, scalars=zstart.ravel()[::skip], opacity=0.8, mode ='sphere', vmin=-3, vmax=3, resolution=16, colormap='rainbow', line_width=1)
pts0.glyph.color_mode = 'color_by_scalar'
pts0.glyph.glyph_source.glyph_source.center = [0, 0, 0]
# pts0.actor.property.edge_visibility = True

mlab.show()

# pts0 = mlab.quiver3d(bt.xstart.ravel()[::skip], bt.ystart.ravel()[::skip], zstart.ravel()[::skip], bsize[::skip], bsize[::skip], bsize[::skip], scale_factor=1.0, scalars=zstart.ravel()[::skip], opacity=0.6, mode ='sphere', vmin=-2, vmax=2, resolution=16)
# pts0.glyph.color_mode = 'color_by_scalar'
# pts0.glyph.glyph_source.glyph_source.center = [0, 0, 0]
#
# f1 = mlab.figure(size=(900, 900), bgcolor=(0.16, 0.28, 0.46))
# s1 = mlab.surf(bt.xcoords, bt.ycoords, surface_hpf, warp_scale=1.0, vmin=-2, vmax=2, colormap='afmhot', opacity = 1)
# s1.actor.mapper.interpolate_scalars_before_mapping = True
# pts1 = mlab.quiver3d(bt.xstart.ravel()[::skip], bt.ystart.ravel()[::skip], zstart.ravel()[::skip], bsize[::skip], bsize[::skip], bsize[::skip], scale_factor=1.0, scalars=zstart.ravel()[::skip], opacity=0.6, mode ='sphere', vmin=-2, vmax=2, resolution=16)
# pts1.glyph.color_mode = 'color_by_scalar'
# pts1.glyph.glyph_source.glyph_source.center = [0, 0, 0]

size = 50
sigma = 5
surface = blt.gauss2d(size, sigma) * 3


rs = 2
dp = 0.2
nt = 50

bt = blt.BT(surface.shape, nt, rs, dp)

xcoords = np.arange(size)
ycoords = np.arange(size)

xstart = 20
ystart = 30
zstart = blt.put_balls_on_surface(surface, xstart, ystart, rs, dp)


pos, vel = blt.initialize_ball_vector(xstart, ystart, zstart)

pos, vel, force = [np.array(v).squeeze() for v in zip(*[blt.integrate_motion(pos, vel, bt, surface) for i in range(nt)])]



f1 = plt.figure(figsize=(10, 10))
plt.imshow(surface, origin='lower', cmap='gray')
plt.plot(xstart, ystart, 'r+', markersize=10)
plt.plot(pos[:,0], pos[:,1], 'go', markerfacecolor='none')





f2 = mlab.figure(size=(900, 900), bgcolor=(0.16, 0.28, 0.46))
# f2.scene.light_manager.lights[0].elevation = - 5
# f2.scene.light_manager.lights[1].elevation = - 80
# f2.scene.light_manager.lights[2].elevation = - 80

s2 = mlab.surf(xcoords, ycoords, g, warp_scale = 1.0, opacity = 1, colormap='afmhot', line_width=1)

mlab.show()




# fig = plt.figure(1)
# ax = fig.add_subplot(111, projection='3d')
# ax.plot_surface(bt.meshx, bt.meshy, surface)
# z = blt.bilin_interp(surface, bt.xstart.ravel(), bt.ystart.ravel())


