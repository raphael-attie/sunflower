...
def get_joblist(file):
    joblist = [line.rstrip('\n').rstrip('TIMEOUT ') for line in open(file)]
    joblist = [int(re.sub(r'.*_', '', line)) for line in joblist]
    return joblist
...
if __name__ == '__main__':
    ...
    tstart = time.time()
    calibrate_partial(bt_params_list[jobs_list[job_id]])
