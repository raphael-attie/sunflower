...
pickled_file = os.path.join(savedir, 'df_datamedn_Sep_2017.pkl')
#df_datamedn = c.query('hmi.Ic_45s[2018.01.01_TAI/90d@6m]', key='T_REC, DATAMEDN')
df_datamedn = c.query('hmi.Ic_45s[2017.08.01_TAI/90d@6m]', key='T_REC, DATAMEDN')
df_datamedn.to_pickle(pickled_file)
df_datamedn2 = df_datamedn.interpolate()
# df_datamedn = pd.read_pickle(pickled_file)
# df_datamedn2 = df_datamedn.interpolate()
...
sig0 = df_datamedn['DATAMEDN'].values # non-interpolated data
sig = df_datamedn2['DATAMEDN'].values
#sig = sig - sig.mean()
...
ax1.set_xlim(xlim)
ax1.grid(True)
ax1.set_title('Median intensity in {:s}'.format(title_date))
ax1.legend(loc='upper left')
ax1R = ax1.twinx()
ax1R.plot(tvec, win, color='gray', ls='--')
...
plt.savefig('/Users/rattie/Data/SDO/TSI/filter_true_sdo_data_{:0.0f}days_fw{:0.0f}em6_{:s}_{:d}d.png'.format(duration / (24*3600), fw*1e6, tsuffix, ndays))
