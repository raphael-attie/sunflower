...
def shift_series(images, outputdir):
    ...
    for dt in dts:
        ...
        # Load image that will be translated
        image = images[..., dt]
        for i, drift in enumerate(drifts):
            shifted_image = filters.translate_by_phase_shift(image, -drift, -drift)
            filename = os.path.join(outputdir_dt, 'time_gap_shifted_df_{:d}_idx_{:02d}.fits'.format(dt, i))
            fitsio.write(filename, shifted_image)


# Create series of translated simulated HMI-like continuum images
outputdir = '/Users/rattie/Data/sanity_check/Stein_simulation/'
...
outputdir = '/Users/rattie/Data/sanity_check/HMI/'
