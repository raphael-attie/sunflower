...
if __name__ == "__main__":
    ...
    datadir = PurePath('/Users/rattie/Data/SDO/HMI/Peter/dataset1')
    datafiles = sorted(glob.glob(str(PurePath(datadir, 'magnetograms', '*.fits'))))
    ...
    outfile = PurePath(datadir, 'mballtrack', 'mballtrack.npz')
    os.makedirs(outfile.parent, exist_ok=True)
    np.savez(outfile, ballpos_p=mbt_p.ballpos, balls_age_p=mbt_p.balls_age_t, valid_balls_p= mbt_p.valid_balls_mask_t,
             ballpos_n=mbt_n.ballpos, balls_age_n=mbt_n.balls_age_t, valid_balls_n= mbt_n.valid_balls_mask_t)

    np.savez_compressed(PurePath(datadir, 'mballtrack', 'mballtrack_compressed.npz'), ballpos_p=mbt_p.ballpos, balls_age_p=mbt_p.balls_age_t, valid_balls_p=mbt_p.valid_balls_mask_t,
             ballpos_n=mbt_n.ballpos, balls_age_n=mbt_n.balls_age_t, valid_balls_n=mbt_n.valid_balls_mask_t)
    ...
    ws_p, markers_p, borders_p = mblt.watershed_series(mbt_dict['datafiles'], mbt_dict['nt'], mbt_dict['noise_level'], 1, mbt_p.ballpos.astype(np.int32))
    ws_n, markers_n, borders_n = mblt.watershed_series(mbt_dict['datafiles'], mbt_dict['nt'], mbt_dict['noise_level'], -1, mbt_n.ballpos.astype(np.int32))

    outfile_watershed = PurePath(datadir, 'mballtrack', 'mballtrack_watershed.npz')
    np.savez(outfile_watershed, ws_p=ws_p, markers_p=markers_p, borders_p=borders_p,
             ws_n=ws_n, markers_n=markers_n, borders_n=borders_n)

    outfile_watershed = PurePath(datadir, 'mballtrack', 'mballtrack_watershed_compressed.npz')
    np.savez_compressed(outfile_watershed, ws_p=ws_p, markers_p=markers_p, borders_p=borders_p,
             ws_n=ws_n, markers_n=markers_n, borders_n=borders_n)
