...
df_data = c.query('hmi.Ic_45s[2017.08.25_TAI/33d@24h]', key='T_REC, DATAMEDN, DATAMEAN, DSUN_OBS, CAMERA')
...
tstart = datetime.date(2017, 8, 24)
tend = datetime.date(2017, 9, 26)


fig = plt.figure(figsize=(18,11))
ax1 = plt.subplot(3,1,1)
df_data2.plot(ax=ax1, y=[data_key, 'dmod_datamean'], xlim=[tstart, tend],
              style='-', grid=True)
ax1.set_xlabel('Date')
...
df_cal2.plot(ax=ax2, y=[data_key, 'dmod_datamean'], style='.', xlim=[tstart, tend], grid=True)
df_cal3.plot(ax=ax2, y=[data_key, 'dmod_mean_binned'], style=[':', '-.'], color='black', xlim=[tstart, tend], grid=True)
ax2.set_ylim([38000, 39500])

ax3 = ax2.twinx()
df_data2.plot(ax=ax3, y=['DSUN_OBS2'], style='--', color='green', xlim=[tstart, tend], grid=True)
...
df_data_sync.plot(ax=ax3, y=['dmod_datamean2'], xlim=[tstart, tend],
              style='-', grid=True)
ax3.set_xlabel('Date')
...
plt.savefig(os.path.join(savedir, 'frames_datamean_with_calibration_Sep_2017.png'))
...
mask = (df_data2.index >= np.datetime64('2017-08-25')) & (df_data2.index <= np.datetime64('2017-09-26'))

df_datamean_detrended = df_data2['DATAMEAN_DETRENDED'].copy().loc[mask]
df_datamean_detrended.to_csv(os.path.join(savedir, 'intensity_mean_detrended_Sep_2017.csv'))


#fig = plt.figure(figsize=(18,11))
ax1 = df_data2.plot(y=['DATAMEAN_DETRENDED'],
              style='-', grid=True, figsize=(18,11))

#ax1.xaxis.set_major_locator(mdates.MonthLocator())
#ax1.xaxis.set_major_formatter(mdates.DateFormatter('%m'))
ax1.set_xticks(df_data2.index)
# ax1.xaxis.set_minor_locator(mdates.DayLocator(interval=2))
# ax1.xaxis.set_minor_formatter(mdates.DateFormatter('%d'))
#ax1.set_xticklabels(df_data2.index.strftime('%b-%d'), rotation=90)
...
plt.savefig(os.path.join(savedir, 'datamean_demodulated_detrended_Sep_2017.png'))

df_data2_feb_2018= df_data2.loc[mask]
del df_data2_feb_2018['CAMERA']
del df_data2_feb_2018['DSUN_OBS2']
df_data2_feb_2018.to_csv(os.path.join(savedir, 'intensity_mean_demodulated_detrended_Sep_2017.csv'), header=True)
